...
#[cfg(test)]
mod tests {
    //! A simulated Apollo host (server side of the handshake, from
    //! `apollo/src/nvhttp.cpp`) run against our client state machine.
    use super::*;
    ...
    impl FakeHost {
        ...
        fn param<'a>(args: &'a str, key: &str) -> Option<&'a str> {
            args.split('&').find_map(|kv| kv.strip_prefix(&format!("{key}=")))
        }

        fn handle(&mut self, step: &Step) -> String {
            let a = &step.args;
            if step.https {
